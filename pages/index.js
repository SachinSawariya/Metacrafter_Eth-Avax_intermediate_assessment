...
export default function HomePage() {
  ...
  const [ownerName, setOwnerName] = useState("Sachin Kumar");
  const [add, setAdd] = useState(14);
  const [sub, setSub] = useState(2);
  const [mult, setMult] = useState(48);
    ...
    return (
      <>
        <div>
          ...
          <button style={{ backgroundColor: "skyblue" }} onClick={withdraw}>
            Withdraw 1 ETH
          </button>
        </div>
  
        <div>
          <h2>Calculator (You can calculate your transaction) </h2>
